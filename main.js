...
// F. CHECKOUT PAGE (Dynamic Summary & COD tax calculations)
function loadCheckoutPage() {
    ...
    // 1. Populate Right Summary Block
    const summaryContainer = document.querySelector('.checkout-container > div');
    const subtotal = cart.reduce((sum, item) => sum + (item.price * item.qty), 0);
    const shipping = 250;
    
    let itemsHtml = cart.map(item => `
        <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; border-bottom: 1px solid #e0e0e0; padding-bottom: 1rem;">
            <div class="summ-thumb" style="border: 1px solid #ddd; overflow: hidden; box-shadow: var(--shadow);">
                <img src="${item.image}" alt="${item.name}" style="width: 100%; height: 100%; object-fit: cover;">
            </div>
            <div style="flex: 1;">
    ...
    summaryContainer.innerHTML = `
        ...
        <div style="border-top: 1px solid #ddd; padding-top: 1.5rem; margin-top: 1.5rem;">
            ...
            <!-- TAX ROW -->
            <div id="taxRow" class="tax-row" style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                <span>COD Advance Surcharge (9%)</span>
                <span id="taxAmount" style="font-weight: bold;">Rs. ${Math.round(subtotal * 0.09).toLocaleString()}</span>
            </div>
            ...
            <div style="display: flex; justify-content: space-between; font-weight: bold; font-size: 1.25rem; margin-top: 1.5rem; border-top: 2px solid var(--black); padding-top: 1rem;">
                ...
                <span id="finalTotal" class="text-gold">Rs. ${(subtotal + shipping + Math.round(subtotal * 0.09)).toLocaleString()}</span>
            </div>
        </div>
    `;

    // 2. Select Payment Toggle bind
    // Ensure inline clicks mapped in checkout.html also call this
    window.selectPay = function(element, type) {
        ...
        if (type === 'cod') {
            ...
            finalTotalEl.innerText = "Rs. " + (subtotal + shipping + taxAmount).toLocaleString();
        } else {
            taxRow.style.display = 'none';
            codWarning.style.display = 'none';
            finalTotalEl.innerText = "Rs. " + (subtotal + shipping).toLocaleString();
        }
    };
    ...
    if (completeOrderBtn) {
        ...
        completeOrderBtn.onclick = async function(e) {
            ...
            const surcharge = paymentType === 'cod' ? Math.round(subtotal * 0.09) : 0;
            const grandTotal = subtotal + shipping + surcharge;
